...
public class Calculator
{
	...
	public static String expression(String exp) {		
		...
		//filter String of multiple character
	...
	public static String Multiple(String exp)
	{
		...
		for(int j=0; j<MulPart.length; j++)
		{
			if(IsDigit(MulPart[j]))
				num *= Integer.parseInt(MulPart[j]);//multiple numbers
			else
	...
	public static String CalculateExp(String exp)
	{
		...
		//first step,make the multiple calculate
	...
	public static String simplify(String exp,String SimpExp)
	{
		char var;
		int val;
		...
		s = Calculator.CalculateExp(s);
		return s;
	}

	public static String derivative(String exp,String DervativeExp)
	{
		String finalExp;

		//get the variable from this expression of derivative
		char var;
		...
		finalExp = Calculator.CalculateExp(finalExp);
		return finalExp;
	}
	...
	public static void main(String[] args) throws IOException 
	{
		...
		//Get the Expression
		exp = Calobject.GetString();
		while(!exp.equals("quit"))
		{
			exp = Calculator.expression(exp);
			System.out.println(exp);

			//input the simplify expression
			String strexp = Calobject.GetString();
			String SimplifyExp = Calculator.expression(strexp);
			...
			{
				SimpExp = Calculator.simplify(exp,SimplifyExp);
				System.out.println(SimpExp);
			}
			else {
				System.out.println(SimplifyExp);
			}
			long SimendTime=System.nanoTime();
			
			//input the derivative expression
			strexp = Calobject.GetString();
			String DervativeExp = Calculator.expression(strexp);
			//If the string "!d/dx" in it,then run derivative()
			long DerstartTime=System.nanoTime();
			strDerivative = DervativeExp.substring(0,2);
			if(strDerivative.equals("!d"))
			{
				DerExp = Calculator.derivative(exp,DervativeExp);
				System.out.println(DerExp);
			}
			else {
				System.out.println(DervativeExp);
			}
